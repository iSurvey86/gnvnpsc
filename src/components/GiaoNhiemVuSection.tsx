...
export function GiaoNhiemVuSection({
  ...
  const soanUrl = soanHref(duAn.id, "tvtk", existing?.id);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col items-center gap-2.5 pt-1 pb-2">
        {needCapForTvtk ? (
          <p className="max-w-xl rounded-xl border border-amber-200 bg-amber-50/80 px-4 py-2 text-center text-xs font-medium text-amber-800">
        ...
        <Link
          href={soanUrl}
          className="inline-flex items-center gap-2.5 rounded-2xl border-2 border-teal-500 bg-teal-50 px-12 py-3.5 text-base font-bold tracking-wide text-teal-800 uppercase shadow-[0_1px_2px_rgba(15,23,42,0.04),0_10px_22px_-14px_rgba(13,148,136,0.45)] transition-all duration-200 hover:-translate-y-0.5 hover:border-teal-600 hover:bg-teal-100 hover:text-teal-900 hover:shadow-[0_2px_4px_rgba(15,23,42,0.06),0_16px_28px_-14px_rgba(13,148,136,0.5)] focus-visible:ring-2 focus-visible:ring-teal-300 focus-visible:outline-none"
        >
          <DesignIcon />
          {existing ? "Mở soạn quyết định" : "Giao nhiệm vụ"}
        </Link>
        {caption ? (
          <p className="text-[11px] font-semibold tracking-wider text-teal-700/70 uppercase">
            {caption}
          </p>
        ) : null}
      </div>
