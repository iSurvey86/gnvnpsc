  ...
  try {
    ...
    if (ids.length) {
      ...
      for (const q of qdXns ?? []) {
        ...
        if (!qdMetaById.has(q.id as string)) {
          qdMetaById.set(q.id as string, {
            id: q.id as string,
    ...
    const assignMap = ganCtKeysChoQdXn({
      phuLucRows,
      duAns: list.map((d) => ({ id: d.id, ten_du_an: d.ten_du_an })),
      qds: qdsForAssign,
    });
    const ctPerQd = demSoCtTheoQd(assignMap);
    ...
    if (phuLucRows.length) {
      cong_trinh = phuLucRows.map((row, i) => {
        ...
        const hit = key ? assignMap.get(key) : undefined;
        return {
          ...
          trang_thai: hit ? qdMetaById.get(hit.qdId)?.trang_thai ?? null : null,
    ...
    const qd_giao_xn: QdXnTheoDoi[] = [...qdMetaById.values()].map((q) => ({
      id: q.id,
      du_an_id: q.du_an_id,
      loai: q.loai,
